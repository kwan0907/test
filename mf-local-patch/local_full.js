...
if(p==='wp'){uniq(f.length?f:b.concat(l)).forEach(function(x){o.push({h:[x],sub:'WIN'});o.push({h:[x],sub:'PLA'})});return o}
if(p==='dbl'){uniq(f.length?f:b.concat(l)).forEach(function(x){uniq(s).forEach(function(y){o.push({h:[x,y]})})});return o}
if(p==='fctb'||p==='fctbm'){if(b.length){b.forEach(function(x){l.forEach(function(y){if(x!==y)o.push({h:[x,y]})})})}else{f=uniq(f);f.forEach(function(x){f.forEach(function(y){if(x!==y)o.push({h:[x,y]})})})}return o}
if(b.length){b.forEach(function(x){l.forEach(function(y){if(x!==y)o.push({h:[Math.min(x,y),Math.max(x,y)]})})})}else{f=uniq(f.length?f:l);for(var i=0;i<f.length;i++)for(var j=i+1;j<f.length;j++)o.push({h:[Math.min(f[i],f[j]),Math.max(f[i],f[j])]})}
...
function parseMatrix(t){var rows=$qa('tr',t).filter(function(r){return $qa('td,th',r).length>=3}),head=[],start=0,map=new Map;if(rows.length<3)return map;for(var ri=0;ri<Math.min(4,rows.length);ri++){var vals=$qa('td,th',rows[ri]).map(function(c){return iv(text(c))}).filter(function(x){return x>=1&&x<=50});if(vals.length>=3){head=vals;start=ri+1;break}}if(!head.length)return map;for(ri=start;ri<rows.length;ri++){var cells=$qa('td,th',rows[ri]),rn=cells.map(function(c){return iv(text(c))}).filter(function(x){return x>=1&&x<=50});if(!rn.length)continue;var row=rn[0],ci=0;for(var q=0;q<cells.length;q++){var v=num(text(cells[q]));if(!(v>1&&v<10000))continue;var col=head[ci++];if(!col||col===row)continue;var key=Math.min(row,col)+'-'+Math.max(row,col);if(!map.has(key))map.set(key,v)}}return map}
...
var syncUI=function(){hideLogin();ensureSmartButton()};var uiObserver=new MutationObserver(function(muts){for(var i=0;i<muts.length;i++){if(muts[i].addedNodes&&muts[i].addedNodes.length){syncUI();break}}});uiObserver.observe(document.documentElement,{childList:true,subtree:true});syncUI();setInterval(syncUI,1500);
})();